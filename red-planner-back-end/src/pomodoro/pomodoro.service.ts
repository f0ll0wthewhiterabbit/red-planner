...
export class PomodoroService {
  ...
  async getTodaySession(userId: string) {
    ...
    return this.prisma.pomodoroSession.findFirst({
      ...
      include: { pomodoroRounds: { orderBy: { id: 'asc' } } },
    })
  }
  ...
  async update(dto: Partial<PomodoroSessionDto>, pomodoroId: string, userId: string) {
    return this.prisma.pomodoroSession.update({ where: { userId, id: pomodoroId }, data: dto })
  }

  async updateRound(dto: Partial<PomodoroRoundDto>, roundId: string) {
    return this.prisma.pomodoroRound.update({ where: { id: roundId }, data: dto })
  }

  async deleteSession(sessionId: string, userId: string) {
    return this.prisma.pomodoroSession.delete({ where: { id: sessionId, userId } })
  }
}
