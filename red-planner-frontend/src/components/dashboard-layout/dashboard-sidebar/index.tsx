'use client'

import { GanttChartSquare } from 'lucide-react'
import Link from 'next/link'

import { COLORS } from '@/constants/color.constants'

import { DashboardLogoutButton } from '../dashboard-logout-button'
import { DashboardMenuItem } from '../dashboard-menu-item'
import { MENU } from '../menu.data'

export function DashboardSidebar() {
  return (
    <aside className='border-r border-r-border h-full bg-sidebar fle flex-col justify-between'>
      <div>
        <Link href='/' className='flex items-center gap-2.5 p-layout border-b border-b-border'>
          <GanttChartSquare color={COLORS.primary} size={38} />
          <span className='text-2xl font-bold relative'>
            RED planner{' '}
            <span className='absolute -top-1 -right-6 text-xs opacity-40 rotate-[18deg] font-normal'>
              beta
            </span>
          </span>
        </Link>
        <div className='p-3 relative'>
          <DashboardLogoutButton />
          {MENU.map(item => (
            <DashboardMenuItem item={item} key={item.link} />
          ))}
        </div>
      </div>
      <footer className='text-xs opacity-40 font-normal text-center p-layout'>
        {new Date().getFullYear()} &copy; With love from{' '}
        <a
          href='https://github.com/f0ll0wthewhiterabbit'
          target='_blank'
          rel='noopener noreferrer'
          className='hover:text-primary text-brand-300 transition-colors'
        >
          f0ll0wthewhiterabbit
        </a>
        . <br /> All rights reserved.
      </footer>
    </aside>
  )
}
